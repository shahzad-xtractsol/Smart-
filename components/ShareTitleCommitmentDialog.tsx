...
interface ShareTitleCommitmentDialogProps {
  open: boolean;
  onClose: () => void;
  titleSearchId: any;
  summaryHtml: string;
...
const ShareTitleCommitmentDialog: React.FC<ShareTitleCommitmentDialogProps> = ({ open, onClose, titleSearchId, summaryHtml, pdfFile }) => {
  ...
  const internalUserForm = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        await titleSearchService.shareTitleCommitment(formData);
        onClose();
      } catch (error) {
  ...
  const handleSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    internalUserForm.setFieldValue('userId', '');
      ...
      <div className="bg-white w-full max-w-2xl mx-4 rounded-lg shadow-lg z-10 overflow-auto">
        <div className="p-4 border-b">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold">Share Title Commitment</h3>
            <button onClick={onClose} className="text-gray-500 hover:text-gray-700">✕</button>
          </div>
        </div>
        <div className="p-4">
          <p className="text-sm mb-3">Share your Title Commitment via message or email</p>

          <div className="bg-[#B9D9EB] p-3 rounded mb-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="font-semibold">Copy Title Commitment to clipboard</div>
                <div className="text-sm text-gray-600">Copy the complete Title Commitment with formatting</div>
              </div>
              <div>
                <button onClick={handleCopyHtml} className="bg-blue-600 text-white px-3 py-1 rounded">Copy Title Commitment</button>
              </div>
            </div>
          </div>
...
export default ShareTitleCommitmentDialog;